...
5. 집계 함수는 WHERE절에는 사용할 수 없지만 HAVING절에는 사용 가능하다. (__X_)
...
2. SQL은 대화식 그리고 (__내장식___)의 2가지 방식으로 사용할 수 있다.
...
7. SELECT절에서 연산이냐 집계 함수의 적용 결과로 새로운 값을 반환할 때 새로운 이름인 별칭을 정의하는 키워드는 (___AS__)이다.
8. (__서브 질의문___)은 질의문 안에 중첩 (nested) 되어 포함된 또 다른 SELECT 검색문을 말한다.
