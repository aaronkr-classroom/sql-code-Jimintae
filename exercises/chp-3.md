...
2. 관계형 데이터베이스(relational database)는 1970년대 IBM 연구소의 (___E. F. Codd___)가 제안한 관계형 데이터 모델에 기반하고 있다.
...
6. 후보키를 포함하는 속성 집합은 모두 (__슈퍼키_)가 된다.
7. 후보키 중에서 적절한 기본키 선정이 어려울 경우, 일련번호 같은 인위적인 속성을 추가로 생성하여 지정하는 기본키를 (__인조키____)라고 한다.
